 ...
 */
const RESERVED_USERNAMES = ["admin", "root", "firebase", "system", "test", "api"];

/**
 * Validar que el username no sea reservado
 */
const isNotReservedUsername = (username: string) => {
  return !RESERVED_USERNAMES.includes(username.toLowerCase());
};

/**
 * Validar que la URL de avatar sea válida
 */
...
export const registerSchema = z.object({
  ...
  lastNames: z
    .string()
    .min(2, "Last names must be at least 2 characters")
    .max(50, "Last names must not exceed 50 characters")
    .trim(),
...
.transform((val) => val.toLowerCase())
    .refine(isNotReservedUsername, "This username is reserved and cannot be used"),
...
/**
 * Schema para completar perfil de Google
 */
